...
import com.sdt.displaycomponents.SixerButton;
 ...
 * @author possi
 */
public class SixerChallengeScreen {
    public static SixerChallengeScreen this_obj=null;
    public static SixerChallengeBean sixerchallengeBean = new SixerChallengeBean();
    Pane pane = null;
    double width =0;
    double height = 0;
    
    public SixerChallengeScreen(Pane pane, double width , double height){
        this_obj=this;
        this.pane = pane;
        this.width=width;
        this.height=height;
    ...
    public static CSpinner<Integer> sixer_to_hit=null;
    public void showScreen(){
        ...
        try {
            ...
            TextType3_0 header = new TextType3_0((width/2)+20, header_font_size, "SIXER CHALLENGE", f_type1, pane,Color.WHITE);
            ...
            bname= new CTextField_Data(sixerchallengeBean.getName(),16);
            ...
            bmob= new CTextField_Data(sixerchallengeBean.getMob(),13);
            bmob.setLayoutX(width*0.25);
            bmob.setLayoutY(height*0.205);
            pane.getChildren().add(bmob);

            TextType1 t_no_of_balls = new TextType1(width*0.54, (height*0.13)+regular_font_size, "NUMBER OF BALLS", f_type2, pane,Color.WHITE);
            balls= new CSpinner(0, 200, sixerchallengeBean.getBalls(), 1);
            balls.setEditable(true);
            balls.setLayoutX(width*0.73);
            balls.setLayoutY(height*0.135);
            pane.getChildren().add(balls);
            TextType1 t_no_of_sixers = new TextType1(width*0.54, (height*0.20)+regular_font_size, "SIXER TO HIT", f_type2, pane,Color.WHITE);
            sixer_to_hit= new CSpinner(0, 120, sixerchallengeBean.getSixers(), 1);
            sixer_to_hit.setEditable(true);
            sixer_to_hit.setLayoutX(width*0.73);
            sixer_to_hit.setLayoutY(height*0.205);
            pane.getChildren().add(sixer_to_hit);

            SettingsButton settings = new SettingsButton(4);
            ...
            SixerButton six = new SixerButton( width*0.15, height*0.15,1);
            six.setLayoutX((width*0.7));
            six.setLayoutY(height*0.35);
            pane.getChildren().add(six);

            SixerButton notsix = new SixerButton( width*0.15, height*0.15,0);
            notsix.setLayoutX((width*0.50));
            notsix.setLayoutY(height*0.35);
            pane.getChildren().add(notsix);
